...
<?php
$saleid = base64_decode($this->input->get('id'));
//echo $saleid;
$this->db->select('sales_order.*,system_user.name,user_name,customer_information.customer_name as cusname,customer_information.mobile as mobile'); //,system_user_type.name as urolename
// $this->db->join("system_user_type","system_user_type.id=sales_order.user_role","left");
$this->db->join("customer_information","sales_order.customer_id=customer_information.id","left");
$this->db->join("system_user","sales_order.add_by=system_user.id","left");
$sales_order = $this->Common_model->get_all('sales_order',['sales_order.id'=> $saleid])->result();
//var_dump($sales_order);
if(!$sales_order):
  die(); 
endif;
$sales_order_payment = $this->Common_model->get_all('sales_order_payment',['order_id'=> $saleid])->result();
...
$sales_order_details = $this->Common_model->get_all('sales_order_details',['sales_order_details.order_id'=> $saleid,'sales_order_details.status'=>1])->result();
       ...
       <div class="" style="width: 300px">
                ...
                <div class="row">
                    ...
                    <div  style="width:60%;"> <?= ($sales_order)?$sales_order[0]->transaction_no :"";    //($sales_order_payment)?$sales_order_payment[0]->transaction_no:"" ?></div>
                     ...
                     <div style="width:40%;font-weight: 600;font-size:14px">Customer </div>
                    <div  style="width:60%;"> <?= ($sales_order)?$sales_order[0]->cusname ." ". $sales_order[0]->mobile:"";    //($sales_order_payment)?$sales_order_payment[0]->transaction_no:"" ?></div>
...
$sales_order_stoles = $this->Common_model->get_all('sales_order_details',['sales_order_details.order_id'=> $saleid,'sales_order_details.status'=>1])->result(); 
?>
